...
export class SnapEngine {
    ...
    calculateSnap(
        ...
        const draggedBounds = this.getElementBounds(
            draggedElement,
            newPosition,
        );
        ...
        const otherBounds = otherElements.map((el) =>
        ...
        for (const otherBound of otherBounds) {
            ...
            if (!hasSnappedX) {
                const xSnap = this.checkHorizontalSnap(
                    draggedBounds,
                    otherBound,
                );
                if (xSnap) {
        ...
        if (!hasSnappedX) {
            const slideXSnap = this.checkSlideEdgeSnapX(
                draggedBounds,
                slideWidth,
            );
            if (slideXSnap) {
        ...
        if (!hasSnappedY) {
            const slideYSnap = this.checkSlideEdgeSnapY(
                draggedBounds,
                slideHeight,
            );
            if (slideYSnap) {
        ...
        if (!hasSnappedX && this.config.enableCenterSnapping) {
            const centerXSnap = this.checkSlideCenterSnapX(
                draggedBounds,
                slideWidth,
            );
            if (centerXSnap) {
    ...
    private checkHorizontalSnap(
        ...
        if (this.config.enableEdgeSnapping) {
            ...
            const rightToRight = Math.abs(
                draggedBounds.right - otherBounds.right,
            );
    ...
    private checkSlideEdgeSnapY(
        ...
        const bottomToSlideBottom = Math.abs(
            draggedBounds.bottom - slideHeight,
        );
    ...
    private checkSlideCenterSnapX(
        ...
        const centerToSlideCenter = Math.abs(
            draggedBounds.centerX - slideCenter,
        );
    ...
    private checkSlideCenterSnapY(
        ...
        const centerToSlideCenter = Math.abs(
            draggedBounds.centerY - slideCenter,
        );
        if (centerToSlideCenter <= tolerance) {
            return {
