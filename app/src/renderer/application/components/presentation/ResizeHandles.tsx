...
interface ResizeHandlesProps {
  ...
  onResize: (elementId: string, updates: { position?: { x: number; y: number }; size?: { width: number; height: number } }) => void;
  minWidth?: number;
  minHeight?: number;
  maxWidth?: number;
  maxHeight?: number;
...
export const ResizeHandles: React.FC<ResizeHandlesProps> = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      switch (resizeDirection) {
        ...
        case 'w': // West (left)
          newWidth = Math.max(minWidth, Math.min(maxWidth, startSize.width - deltaX));
          newX = startElementPosition.x + (startSize.width - newWidth);
          break;
        case 's': // South (bottom)
  ...
  }, [isResizing, resizeDirection, startPosition, startSize, startElementPosition, elementId, onResize, minWidth, minHeight, maxWidth, maxHeight]);
