...
export function useTextMeasurement() {
  const measureText = useCallback(
    (request: TextMeasurementRequest): TextMeasurementResult => {
      const {
        content,
        fontSize,
        fontFamily,
      ...
      try {
        ...
        const lineCount = Math.max(
          1,
          Math.round(actualHeight / singleLineHeight),
        );
   ...
   */
  const measureTextWithSuggestions = useCallback(
    (request: TextMeasurementRequest) => {
      const measurement = measureText(request);

      return {
        ...measurement,
        suggestedWidth: measurement.hasOverflow
          ? measurement.naturalWidth
          : request.width,
        suggestedHeight: measurement.actualHeight,
        isOptimalSize:
          !measurement.hasOverflow && measurement.actualHeight <= request.width,
      };
    },
    [measureText],
  );
