    ...
    useEffect(() => {
        ...
        const threadUpdatedUnsubscribe = electronAPI.ipcRenderer.on(
            ...
            (...args: unknown[]) => {
                ...
                console.log(
                    `Thread updated event for thread: ${updatedThread.id}`,
                );
                ...
                setThreads((prev) => {
                    ...
                    if (index === -1) {
                        console.log(
                            `Adding thread in update event: ${updatedThread.id}`,
                        );
                        return [...prev, updatedThread];
                    }

                    console.log(
                        `Updating thread in threads array: ${updatedThread.id}`,
                    );
        ...
        async (message: string, content?: MessageContent[]) => {
            ...
            try {
                ...
                const updatedThreads =
                    await electronAPI.ai.getThreadsForPresentation(
                        presentationId,
                    );
                ...
                if (
                    ...
                    console.log(
                        'Message sending was aborted - leaving thread as-is',
                    );
