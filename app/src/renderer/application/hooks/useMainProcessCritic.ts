...
interface ElectronWindow {
    electron: {
        ...
        critic: {
            ...
            reviewSlide: (threadId: UUID, slideId: UUID) => Promise<string>;
        };
    };
}
...
export const useMainProcessCritic = (presentationId: UUID) => {
    ...
    const loadThreads = useCallback(async () => {
        ...
        try {
            setIsLoading(true);
            const loadedThreads =
                await electronAPI.critic.getThreadsForPresentation(
                    presentationId,
                );
                ...
                if (data.threadId && data.messageId) {
                    ...
                    if (currentThread?.id === data.threadId) {
                        setCurrentThread((prev) => {
                            ...
                            const messageIndex =
                                updatedThread.messages.findIndex(
                                    (m) => m.id === data.messageId,
                                );
                            if (messageIndex === -1) return prev;
    ...
    const createThread = useCallback(
        async (title: string) => {
            ...
            try {
                ...
                await electronAPI.critic.createThread(title, presentationId);
    ...
    const loadThread = useCallback(async (threadId: UUID) => {
        try {
            ...
            const thread = await electronAPI.critic.getThread(threadId);
    ...
    const deleteThread = useCallback(async (threadId: UUID) => {
        try {
            ...
            const success = await electronAPI.critic.deleteThread(threadId);
