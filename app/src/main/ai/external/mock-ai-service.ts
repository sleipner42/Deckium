...
export class MockAIService implements IAIService {
    async chat(messages: Message[], deploymentName?: string): Promise<string> {
        ...
        if (lastUserMessage) {
            if (typeof lastUserMessage.content === 'string') {
                ...
                const textContents = lastUserMessage.content
                    ...
                    .map(
                        (item) =>
                            (item as MessageContent & { text: string }).text,
                    );
                ...
                const imageContents = lastUserMessage.content
                    ...
                    .map(
                        (item) =>
                            `[Image: ${(item as MessageContent & { image_url: { url: string } }).image_url.url}]`,
                    );
                ...
                if (imageContents.length > 0) {
                    console.log(
                        `Received ${imageContents.length} image(s) in the message`,
                    );
                }
            }
        }
        ...
        console.log(
            `Last user message: ${userMessage.substring(0, 100)}${userMessage.length > 100 ? '...' : ''}`,
        );
        ...
        if (userMessage.toLowerCase().includes('help')) {
            return (
                'I can help you create and manage your presentation. Here are some things I can do:\n\n' +
                '- Create new slides\n' +
        ...
        if (
            userMessage.toLowerCase().includes('slide') ||
            userMessage.toLowerCase().includes('create')
        ) {
            return (
                'I can help with slide creation. Would you like me to create a new slide for you?\n\n' +
                '### Action ###\n' +
        ...
        if (userMessage.toLowerCase().includes('get presentation info')) {
            return (
                'I can help you get information about your presentation. Would you like me to get the presentation info for you?\n\n' +
                '### Action ###\n' +
        ...
        if (
            userMessage.toLowerCase().includes('bar chart') ||
            userMessage.toLowerCase().includes('barchart') ||
            (userMessage.toLowerCase().includes('chart') &&
                !userMessage.toLowerCase().includes('shape'))
        ) {
            return (
                'I can create a bar chart for your slide. Would you like me to create a bar chart?\n\n' +
                '### Action ###\n' +
        ...
        if (
            userMessage.toLowerCase().includes('update chart') ||
            userMessage.toLowerCase().includes('edit chart')
        ) {
            return (
                'I can update an existing bar chart on your slide. Would you like me to update a bar chart?\n\n' +
                '### Action ###\n' +
        ...
        if (
            userMessage.toLowerCase().includes('create shape') ||
            ...
            return (
                `I can create a ${shapeType} shape for your slide. Would you like me to create it?\n\n` +
                '### Action ###\n' +
        ...
        if (
            userMessage.toLowerCase().includes('update shape') ||
            userMessage.toLowerCase().includes('edit shape') ||
            userMessage.toLowerCase().includes('modify shape')
        ) {
            return (
                'I can update an existing shape on your slide. Would you like me to update a shape?\n\n' +
                '### Action ###\n' +
        ...
        if (
            userMessage.toLowerCase().includes('align') ||
            ...
            if (userMessage.toLowerCase().includes('top')) {
                ...
                userMessage.toLowerCase().includes('distribute') &&
                userMessage.toLowerCase().includes('horizontal')
            ) {
                alignType = 'distribute-horizontal';
            } else if (
                userMessage.toLowerCase().includes('distribute') &&
                userMessage.toLowerCase().includes('vertical')
            ) {
            ...
            const includeReference =
                userMessage.toLowerCase().includes('reference') ||
            ...
            const referenceParam = includeReference
                ? ', "referenceElementId": "REFERENCE_ELEMENT_ID"'
                : '';

            return (
                `I can help align elements on your slide. Would you like me to align them ${alignType.replace('-', ' ')}?\n\n` +
                '### Action ###\n' +
    ...
    async chatStream(
        ...
        console.log(
            `Streaming response for message: ${userMessage.substring(0, 100)}${userMessage.length > 100 ? '...' : ''}`,
        );
        ...
        if (
            userMessage.toLowerCase().includes('presentation') ||
            userMessage.toLowerCase().includes('create slides') ||
            userMessage.toLowerCase().includes('make slides')
        ) {
            // First, get presentation info
            response =
                "I'll help you create a presentation. Let me first check what we're working with.\n\n" +
                '### Action ###\n' +
                '{ "tool": "getPresentationInfo", "params": {} }';
        ...
        if (
            userMessage.toLowerCase().includes('tool execution completed') &&
            messages.some(
                (m) =>
                    m.content &&
                    typeof m.content === 'string' &&
                    m.content.includes('getPresentationInfo'),
            )
        ) {
            response =
                "Now I'll create a new slide for your presentation. I'll make it about the key benefits of your product.\n\n" +
                '### Action ###\n' +
        ...
        if (
            ...
            messages.some(
                (m) =>
                    m.content &&
                    typeof m.content === 'string' &&
                    m.content.includes('createSlide'),
            )
        ) {
            response =
                "Great! I've created a slide with the title 'Key Benefits'. Now, let me add some bullet points to it.\n\n" +
                '### Action ###\n' +
        ...
        if (
            ...
            messages.some(
                (m) =>
                    m.content &&
                    typeof m.content === 'string' &&
                    m.content.includes('addTextElement'),
            )
        ) {
            response =
                "Now I'll add a shape to highlight the important information.\n\n" +
                '### Action ###\n' +
        ...
        if (
            ...
            messages.some(
                (m) =>
                    m.content &&
                    typeof m.content === 'string' &&
                    m.content.includes('createShape'),
            )
        ) {
            response =
                "I've completed the slide with a title, bullet points, and a background shape. The slide looks great! Now let's make sure everything is properly aligned.\n\n" +
                '### Action ###\n' +
