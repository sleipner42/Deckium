...
describe('AddTextElementTool', () => {
  ...
  describe('Text Dimension Estimation', () => {
    it('should estimate and adjust height for long text', async () => {
      const longText = 'This is a very long text that should wrap multiple lines and require height adjustment to fit properly within the specified width of the text element container.';
  ...
  describe('Error Handling', () => {
    it('should handle service failures gracefully', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe(`Failed to add element to slide with ID ${slideId}`);
    });
  });
