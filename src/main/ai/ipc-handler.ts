...
import { AIRequest, AIResponse, Thread } from '../../common/domain/entities/ai-types';
import { UUID } from '../../common/domain/entities/types';

export function setupAIIPC(service: AIService) {
  
  ipcMain.handle('ai:create-thread', (_, title: string, presentationId: UUID): Thread => {
  ...
  ipcMain.handle('ai:get-threads-for-presentation', (_, presentationId: UUID): Thread[] => {
  ...
  ipcMain.handle('ai:send-message', (_, request: AIRequest): Promise<AIResponse> => {
