...
import { IAIService, IAIServiceFactory, IAIModelClient, MessageContent } from '../../../common/domain/interfaces/ai-service.interface';
...
export class AzureOpenAIService implements IAIService {
  ...
  async chat(messages: Message[], deploymentName?: string): Promise<string> {
    ...
    try {
      ...
      if (response.usage) {
        console.log(`[AzureOpenAI] Token usage - Prompt: ${response.usage.prompt_tokens}, Completion: ${response.usage.completion_tokens}, Total: ${response.usage.total_tokens}`);
      }
...
export class AzureOpenAIServiceFactory implements IAIServiceFactory {
  createService(): IAIService {
    ...
    try {
      ...
      const initEndTime = performance.now();
      console.error(`[AzureOpenAI] Failed to initialize client (after ${initEndTime - initStartTime}ms):`, error);
      throw error;
    }
  }
}